...
pgp_sign_target="${INPUT_PGP_SIGN_TARGET:-}"
pgp_sign_checksum=''
pgp_sign_asset=''
if [[ -n "${pgp_sign_target}" ]]; then
    while read -rd,; do
        case "${REPLY}" in
            checksum) pgp_sign_checksum='1' ;;
            asset) pgp_sign_asset='1' ;;
            *) bail "'pgp_sign_target' input option must be 'checksum', 'archive', or 'asset': '${REPLY}'" ;;
        esac
    done <<<"${pgp_sign_target},"
fi
...
if [[ -n "${pgp_public_key}" ]]; then
    ...
    for asset in "${assets[@]}"; do
        case "${asset}" in
            *.sha256 | *.sha512 | *.sha1 | *.md5)
                if [[ -z "${pgp_sign_checksum}" ]]; then
                    continue
                fi
                ;;
            *)
                if [[ -z "${pgp_sign_asset}" ]]; then
                    continue
                fi
                ;;
        esac
